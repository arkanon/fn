...
### PARAMETROS ###
  ...
  if [ $(uname -p) = x86_64 ]
  then
           # limite recursão com bfloat 2x:
           #   32 bit -  118
           #   64 bit - 1979
     n=500 # quantidade de graficos (limitada pela quantidade maxima de recursoes), plotados 2 a 2
    nl=20  # #frames lentos
  else
     n=40
    nl=10
...
do
  b=$(printf $pt $((n-10#$a)))
  ln -s $(basename $prefix)-$a.png $prefix-$b.png
done
